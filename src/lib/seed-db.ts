  ...
  for (const seed of PRODUCT_SEEDS) {
    await client.product.upsert({
      ...
      update: {
        ...
        description: `${seed.targetBuyer}\n\n${seed.description}`,
        features: JSON.stringify(seed.features),
        specs: JSON.stringify({ ...seed.specs, "Key Params": seed.keyParams.join(" · ") }),
        scenarios: JSON.stringify(seed.scenarios),
        accessories: JSON.stringify(seed.accessories),
        delivery: JSON.stringify(seed.delivery),
        maintenance: JSON.stringify(seed.deploymentNotes),
        faq: JSON.stringify([
          ...seed.faq,
          ...seed.customizationOptions.map((o) => ({ q: `Customization: ${o}`, a: "Available on request — include in RFQ." })),
        ]),
