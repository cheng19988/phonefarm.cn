...
export function ContactBar({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`flex flex-wrap items-center gap-x-4 gap-y-1 ${compact ? "text-xs" : "text-sm"}`}>
      ...
      <a href={`mailto:${CONTACT.email}`} className="text-slate-300 hover:text-amber-300 transition-colors">
        {CONTACT.email}
      </a>
...
export function ContactCTA({ title = "Need a phone farm configuration?" }: { title?: string }) {
  return (
    <section className="card-flat bg-gradient-to-br from-[var(--surface-elevated)] to-[var(--surface)] border-amber-500/20">
  ...
  return (
    <div className="fixed bottom-0 inset-x-0 z-40 md:hidden bg-[#070a0f]/95 backdrop-blur border-t border-[var(--border)]">
      <div className="grid grid-cols-4 divide-x divide-[var(--border)]">
        ...
        <Link href="/contact" className="flex flex-col items-center justify-center py-3 min-h-[48px] text-[11px] text-amber-400 font-semibold">
          RFQ
        </Link>
      </div>
    </div>
